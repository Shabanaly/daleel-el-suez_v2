                        ...
                        <div className="flex overflow-x-auto hide-scrollbar gap-2 pb-2 -mx-4 px-4 scroll-smooth">
                            {categories.map(cat => (
                                <button
                                    ...
                                    className={`shrink-0 px-5 md:px-7 py-2.5 md:py-3 rounded-full text-sm md:text-base font-bold transition-all duration-300 border-2 flex items-center gap-2 ${activeCategory === cat.name
                                ...
                                >
                                    <span>{cat.name}</span>
                                    <span className={`text-[10px] md:text-xs font-black px-1.5 py-0.5 rounded-md transition-colors ${activeCategory === cat.name
            ...
            <div className="max-w-5xl mx-auto px-4 mb-6">
                <div className="inline-flex items-center gap-3 px-5 py-2.5 bg-surface/80 border border-border-subtle shadow-sm rounded-2xl">
                    <div className="flex items-baseline gap-1.5 leading-none">
                        <span className="text-primary font-black text-lg md:text-xl">{total}</span>
                    ...
                    {(activeCategory !== 'الكل' || activeDistrict !== 'كل الأحياء' || activeArea !== 'كل المناطق') && (
                        <div className="flex items-center gap-2 text-xs md:text-sm font-medium text-text-muted border-r-2 border-border-subtle pr-3 mr-1">
                            ...
                            {(activeDistrict !== 'كل الأحياء' || activeArea !== 'كل المناطق') && (
                                <span className="opacity-40">·</span>
                            )}
                            {activeDistrict !== 'كل الأحياء' && <span>{activeDistrict}</span>}
                            {activeArea !== 'كل المناطق' && <span><span className="mx-1">/</span>{activeArea}</span>}
                        </div>
                    )}
                </div>
            </div>
