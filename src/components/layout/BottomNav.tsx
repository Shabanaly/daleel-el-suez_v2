    ...
    <>
      <div className="lg:hidden fixed bottom-0 w-full z-50 pointer-events-auto flex justify-center">
        <div className="relative w-full max-w-sm md:max-w-2xl lg:max-w-4xl h-[76px]">
          ...
          <div className="relative z-10 flex items-center justify-between h-full px-2 md:px-6">
            ...
            <div className="flex items-center justify-evenly flex-1 h-full">
              <NavItem
                ...
                icon={<Users />}
                label="المجتمع"
                active={pathname === ROUTES.COMMUNITY}
              />
              <NavItem
                href={ROUTES.MARKET}
                icon={<Store />}
                label="السوق"
                active={pathname === ROUTES.MARKET}
              />
            </div>
          </div>
          ...
          <div
            className="absolute left-1/2 -translate-x-1/2 -top-4 w-[60px] flex flex-col items-center justify-center group cursor-pointer z-20"
...
function NavItem({
  ...
  return (
    ...
    >
      {active && (
        <div className="absolute inset-0 flex flex-col items-center pointer-events-none overflow-hidden">
          ...
          <div
            ...
            style={{
              clipPath:
                'path("M 15,0 L 65,0 C 60,30 55,50 40,65 C 25,50 20,30 15,0 Z")',
            }}
          />
        </div>
      )}
      ...
      >
        <div
          className={`relative [&>svg]:w-6 [&>svg]:h-6 [&>svg]:transition-all [&>svg]:duration-500 ${
            active
              ? "text-primary"
      ...
      <span
        className={`relative z-10 text-[10px] sm:text-[11px] mt-1 transition-all duration-300 tracking-wide ${
          active
