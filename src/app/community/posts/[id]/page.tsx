...
export default async function PostPage({ params }: PostPageProps) {
    ...
    const breadcrumbs = [
        { name: 'الرئيسية', item: '/' },
        { name: 'مجتمع السويس', item: '/community' },
        { name: post.category_name || 'نقاش', item: `/community?category=${encodeURIComponent(post.category_id || '')}` },
        { name: post.content.substring(0, 30), item: `/community/posts/${post.id}` }
    ];

    return (
        <>
            <AppBar title={post.content.substring(0, 50)} />
